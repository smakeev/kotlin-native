...
namespace {
    ...
    class GCOVProfilerPatched : public ModulePass {

    public:
        static char ID;

        GCOVProfilerPatched() : GCOVProfilerPatched(GCOVOptions::getDefault()) {}

        GCOVProfilerPatched(const GCOVOptions &Opts) : Options(Opts), ModulePass(ID) {
    ...
    private:
        ...
        Function *insertFlush(ArrayRef<std::pair<GlobalVariable *, MDNode *>>, Function *WriteoutF);

        void insertIndirectCounterIncrement();

        enum class GCovFileType {
            GCNO, GCDA
        };
    ...
    private:
        GCOVProfilerPatched Profiler;
    };
}

char GCOVProfilerPatched::ID = 0;
...
std::string GCOVProfilerPatched::mangleName(const DICompileUnit *CU,
...
bool GCOVProfilerPatched::runOnModule(Module &M) {
    this->M = &M;
    Ctx = &M.getContext();
...
PreservedAnalyses GCOVProfilerPass::run(Module &M,
    ...
    GCOVProfilerPatched Profiler(GCOVOpts);
...
void GCOVProfilerPatched::emitProfileNotes() {
    ...
    for (unsigned i = 0, e = CU_Nodes->getNumOperands(); i != e; ++i) {
        ...
        SmallVector<std::unique_ptr<GCOVFunction>, 16> CUFuncs;
        ...
        for (auto &F : M->functions()) {
            ...
            if (SP->getFile() != CU->getFile()) continue;
            ...
            CUFuncs.push_back(make_unique<GCOVFunction>(SP, &F, &out, FunctionIdent++,
                                                        Options.UseCfgChecksum,
                                                        Options.ExitBlockBeforeBody));
            GCOVFunction &Func = *CUFuncs.back();
...
bool GCOVProfilerPatched::emitProfileArcs() {
    NamedMDNode *CU_Nodes = M->getNamedMetadata("llvm.dbg.cu");
    ...
    Function *FlushF = insertFlush(CountersBySP, WriteoutF);
...
GlobalVariable *GCOVProfilerPatched::buildEdgeLookupTable(
        Function *F,
        GlobalVariable *Counters,
        const UniqueVector<BasicBlock *> &Preds,
        const UniqueVector<BasicBlock *> &Succs) {
...
Constant *GCOVProfilerPatched::getStartFileFunc() {
    Type *Args[] = {
...
Constant *GCOVProfilerPatched::getIncrementIndirectCounterFunc() {
...
Constant *GCOVProfilerPatched::getEmitFunctionFunc() {
    Type *Args[] = {
...
Constant *GCOVProfilerPatched::getEmitArcsFunc() {
    Type *Args[] = {
...
Constant *GCOVProfilerPatched::getSummaryInfoFunc() {
    FunctionType *FTy = FunctionType::get(Type::getVoidTy(*Ctx), false);
    return M->getOrInsertFunction("llvm_gcda_summary_info", FTy);
}

Constant *GCOVProfilerPatched::getEndFileFunc() {
    FunctionType *FTy = FunctionType::get(Type::getVoidTy(*Ctx), false);
    return M->getOrInsertFunction("llvm_gcda_end_file", FTy);
}

GlobalVariable *GCOVProfilerPatched::getEdgeStateValue() {
...
Function *GCOVProfilerPatched::insertCounterWriteout(
...
void GCOVProfilerPatched::insertIndirectCounterIncrement() {
    Function *Fn =
            cast<Function>(GCOVProfilerPatched::getIncrementIndirectCounterFunc());
...
static RegisterPass<GCOVProfilerPatched>
        X("insert-gcov-profiling-patched", "Insert instrumentation for GCOV profiling", false, false);
